//! Shader validation helpers: capability mapping and naga module validation.
//!
//! This module houses the logic that converts wgpu feature/downlevel flags into
//! naga validation capabilities, as well as helpers that map between wgpu texture
//! formats and naga storage formats.
...
/// Map a wgpu [`TextureFormat`] to the corresponding naga [`StorageFormat`].
///
/// Returns `None` for formats that have no naga storage format equivalent.
///
/// [`TextureFormat`]: wgt::TextureFormat
/// [`StorageFormat`]: naga::StorageFormat
...
/// Map a naga [`StorageFormat`] to the corresponding wgpu [`TextureFormat`].
///
/// [`StorageFormat`]: naga::StorageFormat
/// [`TextureFormat`]: wgt::TextureFormat
